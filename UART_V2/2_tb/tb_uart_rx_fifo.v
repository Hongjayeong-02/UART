...
task send_byte;
        ...
        begin
                ...
                // Wait until RX receives this byte
                @(posedge w_rxValid);

                // RX FIFO writes on following clock
                repeat (3)
                        @(posedge clk);
...
task fifo_read_check;
        ...
        begin
                ...
                if (w_fifoEmpty) begin

                        $display(
                                "FAIL: FIFO EMPTY before read"
                        );
                ...
                if (w_rdData !== expectedRxData) begin
...
initial begin
        ...
        if (w_fifoCount !== 5'd4) begin

                $display(
                        "FAIL: FIFO COUNT expected=4 actual=%0d",
                        w_fifoCount
                );
        ...
        fifo_read_check(8'h12);
        fifo_read_check(8'h34);
        ...
        if (w_fifoCount !== 5'd3) begin

                $display(
                        "FAIL: FIFO COUNT expected=3 actual=%0d",
                        w_fifoCount
                );
        ...
        fifo_read_check(8'h5A);
        fifo_read_check(8'hA5);
        fifo_read_check(8'h3C);
        ...
        fifo_read_check(8'h35);
        fifo_read_check(8'h2A);
        fifo_read_check(8'h4D);
...
initial begin
        ...
        $display("");
        $display("========================================");
        $display(" FAIL: RX FIFO SIMULATION TIMEOUT");
        $display("========================================");
        $display("");
